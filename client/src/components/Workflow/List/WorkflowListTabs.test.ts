...
vi.mock("@/composables/config", async () => {
    ...
    return {
        useConfig: () => ({
            config: computed(() => ({ curated_workflows_enabled: configState.curatedEnabled })),
...
async function mountTabs(active: WorkflowListTab, options: MountOptions = {}) {
    const { anonymous = false, curatedEnabled = true, configLoaded = true } = options;

    configState.curatedEnabled = curatedEnabled;
...
describe("WorkflowListTabs", () => {
    ...
    it("hides the curated tab when curated workflows are disabled", async () => {
        const wrapper = await mountTabs("published", { curatedEnabled: false });
