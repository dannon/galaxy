        ...
        if dataset.has_data():
            for site_name, site_url in app.datatypes_registry.get_legacy_sites_by_build("ucsc", dataset.dbkey):
                if site_name in app.datatypes_registry.get_display_sites("ucsc"):
                    ...
                    sl.append(
                        "&hgGenome_dataSetName={}&hgGenome_dataSetDescription={}".format(dataset.name, "GalaxyGG_data")
                    )
...
class Rgenetics(Html):
    ...
    def set_meta(self, dataset, **kwd):
        ...
        try:
            ...
            if verbose:
                gal_Log.debug(
                    "@@@rgenetics set_meta failed {} - dataset {} has no efp ?".format(sys.exc_info()[0], dataset.name)
                )
            return False
        try:
            flist = os.listdir(efp)
