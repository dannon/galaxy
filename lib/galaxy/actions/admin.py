...
class AdminActions:
    ...
    def _create_quota(self, params, decode_id=None):
        ...
        if not params.name or not params.description:
            ...
            self.sa_session.query(self.app.model.Quota).filter(self.app.model.Quota.table.c.name == params.name).first()
        ):
            ...
            params.default != "no"
            and params.default not in self.app.model.DefaultQuotaAssociation.types.__dict__.values()
        ):
            ...
            if params.default != "no":
                ...
                in_users = [
                    self.sa_session.query(self.app.model.User).get(decode_id(x) if decode_id else x)
    ...
    def _set_quota_default(self, quota, params):
        if (
            ...
            if params.default != "no":
                ...
                if quota.default:
                    message = "Quota '{}' is no longer the default for {} users.".format(
    ...
    def _delete_quota(self, quota, params=None):
        ...
        if len(names) == 1:
            raise ActionInputError(
                "Quota '%s' is a default, please unset it as a default before deleting it." % (names[0])
            )
        elif len(names) > 1:
            raise ActionInputError(
                "Quotas are defaults, please unset them as defaults before deleting them: " + ", ".join(names)
            )
