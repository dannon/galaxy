...
def int_to_octet(size):
    ...
    try:
        ...
        return "{:.0f} {}".format(size * ((no_unit - len(units) + 1) * 1000.0), units[-1])
...
class History(BaseUIController):
    ...
    def history_and_dataset_per_user(self, trans, **kwd):
        ...
        histories = {_.email if _.email is not None else "Unknown": int(_.history) for _ in histories.execute()}
        datasets = {
            _.email if _.email is not None else "Unknown": (int(_.dataset), int(_.size)) for _ in datasets.execute()
        }
