...
from galaxy.managers import api_keys, base as managers_base, configuration, users, workflows
    ...
    def get_object(self, trans, id, class_name, check_ownership=False, check_accessible=False, deleted=None):
        try:
            ...
            raise HTTPBadRequest(
                detail="Invalid {} id ( {} ) specified: {}".format(class_name, str(id), util.unicodify(e))
            )
...
class UsesLibraryMixin:
    def get_library(self, trans, id, check_ownership=False, check_accessible=True):
        ...
        if check_accessible and not (
            trans.user_is_admin or trans.app.security_agent.can_access_library(trans.get_current_user_roles(), library)
        ):
...
class SharableMixin:
    ...
    def sharing(self, trans, id, payload=None, **kwd):
        ...
        if item.importable and not item.slug:
            self._make_item_accessible(trans.sa_session, item)
        item_dict = self.serializer.serialize_to_view(item, user=trans.user, trans=trans, default_view="sharing")
        item_dict["users_shared_with"] = [
            {"id": self.app.security.encode_id(a.user.id), "email": a.user.email} for a in item.users_shared_with
        ]
        if skipped:
