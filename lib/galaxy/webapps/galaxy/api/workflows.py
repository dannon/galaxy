...
from galaxy.webapps.base.controller import BaseAPIController, SharableMixin, url_for, UsesStoredWorkflowMixin
...
class WorkflowsAPIController(BaseAPIController, UsesStoredWorkflowMixin, UsesAnnotations, SharableMixin):
    ...
    def get_workflows_list(
        self, trans, missing_tools=False, show_published=None, show_hidden=False, show_deleted=False, **kwd
    ):
        """
    ...
    def index_invocations(self, trans, workflow_id=None, **kwd):
        ...
        if encoded_history_id:
            history = self.history_manager.get_accessible(
                self.decode_id(encoded_history_id), trans.user, current_history=trans.history
            )
    ...
    def _generate_invocation_bco(self, trans, invocation_id, **kwd):
        ...
        bco_dict.update(
            {
                "object_id": url_for(
                    controller="api/invocations/%s" % invocation_id, action="biocompute", qualified=True
                ),
