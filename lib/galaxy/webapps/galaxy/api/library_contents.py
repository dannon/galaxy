...
from galaxy.model import ExtendedMetadata, ExtendedMetadataIndex, tags
...
class LibraryContentsController(
    BaseAPIController, UsesLibraryMixin, UsesLibraryMixinItems, UsesFormDefinitionsMixin, LibraryActions
):
    ...
    def create(self, trans, library_id, payload, **kwd):
        ...
        if create_type == "file":
            ...
            return [
                dictify_dataset_collection_instance(dataset_collection_instance, security=trans.security, parent=parent)
            ]
        if status != 200:
    ...
    def _upload_library_dataset(self, trans, library_id, folder_id, **kwd):
        ...
        if replace_id not in ["", None, "None"]:
            replace_dataset = trans.sa_session.query(trans.app.model.LibraryDataset).get(
