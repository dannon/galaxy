...
def render_item_hda(trans, hda, children):
    ...
    if hda.copied_from_history_dataset_association:
        ...
        template = (
            '<div id="hda-' + trans.security.encode_id(hda.id) + '" class="dataset hda state-' + hda.state + '"></div>'
        )
    return template
    ...
    for input in input_params.values():
        if input.name in param_values:
            if input.type == "repeat":
                ...
                tool_parameter_template += (
                    "<td>" + str(len(param_values[input.name])) + " uploaded datasets</td><td></td></tr>"
                )
                ...
                for i, element in enumerate(listify(param_values[input.name])):
                    ...
                    if element.history_content_type == "dataset":
                        ...
                        tool_parameter_template += (
                            'href="' + dataset_info_url + '">' + str(hda.hid) + ":" + hda.name + "</a>"
                        )
                    else:
