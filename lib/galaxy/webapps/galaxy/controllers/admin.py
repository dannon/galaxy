...
class AdminGalaxy(controller.JSAppLauncher, AdminActions, UsesQuotaMixin, QuotaParamParser):
            ...
            try:
                ...
                if user:
                    ...
                    return trans.show_message(
                        'You are now logged in as {}, <a target="_top" href="{}">return to the home page</a>'.format(
    ...
    def check_for_tool_dependencies(self, trans, migration_stage):
        ...
        tools_xml_file_path = os.path.abspath(
            os.path.join(common_util.TOOL_MIGRATION_SCRIPTS_DIR, "%04d_tools.xml" % migration_stage)
        )
        ...
        if shed_url:
            for elem in root:
                if elem.tag == "repository":
                    ...
                    text = url_get(
                        shed_url, auth=self.app.tool_shed_registry.url_auth(shed_url), pathspec=pathspec, params=params
                    )
                    if text:
    ...
    def review_tool_migration_stages(self, trans, **kwd):
        ...
        migration_scripts_dir = os.path.abspath(
            os.path.join(trans.app.config.root, "lib", "galaxy", "tool_shed", "galaxy_install", "migrate", "versions")
        )
    ...
    def job_info(self, trans, jobid=None):
        ...
        return trans.fill_template("/webapps/reports/job_info.mako", job=job)
    ...
    def sanitize_allowlist(self, trans, submit_allowlist=False, tools_to_allowlist=None):
        ...
        return trans.fill_template(
            "/webapps/galaxy/admin/sanitize_allowlist.mako",
