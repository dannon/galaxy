...
class HistoryDatasetsSelectionGrid(grids.Grid):
    class DbKeyColumn(grids.GridColumn):
        def filter(self, trans, user, query, dbkey):
            ...
            return query.filter(
                or_(
                    text('metadata like \'%%"dbkey": ["%s"]%%\'' % dbkey, 'metadata like \'%%"dbkey": "%s"%%\'' % dbkey)
                )
            )
    ...
    def imp(self, trans, id):
        ...
        if visualization.importable is False:
            return trans.show_error_message(
                "The owner of this visualization has disabled imports via this link.<br>You can %s" % referer_message,
                use_panels=True,
            )
        elif visualization.deleted:
            return trans.show_error_message(
                "You can't import this visualization because it has been deleted.<br>You can %s" % referer_message,
                use_panels=True,
            )
        else:
    ...
    def share(self, trans, id=None, email="", use_panels=False):
        ...
        if email:
            other = (
                ...
                .filter(and_(model.User.table.c.email == email, model.User.table.c.deleted == false()))
                .first()
            )
            if not other:
                mtype = "error"
