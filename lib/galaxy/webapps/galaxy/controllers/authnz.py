...
class OIDC(JSAppLauncher):
    ...
    def index(self, trans, **kwargs):
        ...
        for token in trans.user.custos_auth:
            ...
            rtv.append(
                {"id": trans.app.security.encode_id(token.id), "provider": token.provider, "email": userinfo["email"]}
            )
        return rtv
    ...
    def create_user(self, trans, provider, **kwargs):
        try:
            ...
            return trans.response.send_redirect(
                trans.request.base + url_for("/") + "root/login?message=" + (str(e) or "Duplicate Email")
            )
