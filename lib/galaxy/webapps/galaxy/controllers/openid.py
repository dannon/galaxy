...
class OpenID(BaseUIController):
    ...
    def openid_auth(self, trans, **kwd):
        ...
        process_url = trans.request.base.rstrip("/") + url_for(
            controller="openid", action="openid_process", openid_provider=openid_provider
        )
        request = None
        try:
            ...
            if request is None:
                return trans.show_error_message(
                    "No OpenID services are available at %s." % openid_provider_obj.op_endpoint_url
                )
        ...
        if request is not None:
            ...
            if request.shouldSendRedirect():
                ...
                form = request.htmlMarkup(
                    trans.request.base, process_url, form_tag_attrs={"id": "openid_message", "target": "_top"}
                )
                self.openid_manager.persist_session(trans, consumer)
                return form
    ...
    def openid_process(self, trans, **kwd):
        ...
        if not trans.app.config.enable_openid:
            return trans.show_error_message(
                "OpenID authentication is not enabled in this instance of Galaxy. %s" % return_link
            )
        consumer = self.openid_manager.get_consumer(trans)
