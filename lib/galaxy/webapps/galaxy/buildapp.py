...
def app_factory(global_conf, load_app_kwds=None, **kwargs):
    ...
    webapp.add_route(
        "/u/{username}/d/{slug}/{filename}", controller="dataset", action="display_by_username_and_slug", filename=None
    )
...
def populate_api_routes(webapp, app):
    ...
    webapp.mapper.connect(
        "/api/histories/{id}/sharing", action="sharing", controller="histories", conditions=dict(method=["GET", "POST"])
    )
    webapp.mapper.connect(
        "dynamic_tool_confs",
    ...
    webapp.mapper.connect(
        "/api/pages/{id}/sharing", action="sharing", controller="pages", conditions=dict(method=["GET", "POST"])
    )
    webapp.mapper.resource(
        "revision",
        "revisions",
