...
def get_anaconda_url(container, anaconda_channel="bioconda"):
    ...
    return "https://anaconda.org/{}/{}/{}/download/linux-64/{}.tar.bz2".format(
        anaconda_channel, name[0], name[1], "-".join(name)
    )
...
def get_alternative_versions(filepath, filename, recipes_path=None, github_repo="bioconda/bioconda-recipes"):
    ...
    for subfile in r:
        if subfile["type"] == "dir":
            if (
                requests.get(
                    "https://raw.githubusercontent.com/{}/master/{}/{}".format(github_repo, subfile["path"], filename)
                ).status_code
                == 200
            ):
...
def deep_test_search(
    ...
    for f in [
        (
            ...
            open_recipe_file,
            ("recipes/{}/{}/meta.yaml".format(name[0], name[1]), recipes_path, github_repo),
            container,
        ),
        (
            get_run_test,
            open_recipe_file,
            ("recipes/{}/{}/run_test.sh".format(name[0], name[1]), recipes_path, github_repo),
            container,
        ),
        (
            get_commands_from_yaml,
            open_recipe_file,
            ("recipes/%s/meta.yaml" % name[0], recipes_path, github_repo),
            container,
        ),
...
def hashed_test_search(
    ...
    githubpage = requests.get(
        "https://raw.githubusercontent.com/BioContainers/multi-package-containers/master/combinations/%s.tsv"
        % container
    )
