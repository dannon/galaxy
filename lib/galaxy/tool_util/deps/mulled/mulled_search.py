...
class GitHubSearch:
    ...
    def get_json(self, search_string):
        ...
        response = requests.get(
            "https://api.github.com/search/code?q=%s+in:path+repo:bioconda/bioconda-recipes+path:recipes"
            % search_string
        ).json()
        return response
    ...
    def recipe_present(self, search_string):
        ...
        if (
            requests.get(
                "https://api.github.com/repos/bioconda/bioconda-recipes/contents/recipes/%s" % search_string
            ).status_code
            == 200
        ):
            return True
