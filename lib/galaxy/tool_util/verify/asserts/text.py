...
def assert_has_text(output, text, n=None):
    ...
    if n is None:
        ...
        assert len(matches) == int(n), "Expected {} matches for '{}' in output file (output '{}'); found {}".format(
            n, text, output, len(matches)
        )
...
def assert_has_line(output, line, n=None):
    ...
    if n is None:
        ...
        assert len(matches) == int(
            n
        ), "Expected {} lines matching '{}' in output file (output was '{}'); found {}".format(
            n, line, output, len(matches)
        )
