    ...
    def url_download(self, install_dir, downloaded_file_name, download_url, extract=True, checksums=None):
        ...
        if "sha256sum" in checksums or "#sha256#" in download_url:
            ...
            if downloaded_checksum != expected:
                raise Exception(
                    f"Given sha256 checksum does not match with the one from the downloaded file ({downloaded_checksum} != {expected})."
                )
        ...
        if "md5sum" in checksums or "#md5#" in download_url or "#md5=" in download_url:
            ...
            if downloaded_checksum != expected:
                raise Exception(
                    f"Given md5 checksum does not match with the one from the downloaded file ({downloaded_checksum} != {expected})."
                )
        ...
        if downloaded_filename and os.path.exists(os.path.join(work_dir, downloaded_filename)):
            if target_directory:
                target_directory = os.path.realpath(
                    os.path.normpath(os.path.join(install_environment.install_dir, target_directory))
                )
...
class SetEnvironment(RecipeStep):
    ...
    def handle_environment_variables(
        self, install_environment, tool_dependency, env_var_dict, set_prior_environment_commands
    ):
        """
    ...
    def execute_step(
        ...
        with lcd(current_dir):
            with settings(warn_only=True):
                ...
                env_file_builder.append_line(
                    name="PATH", action="prepend_to", value=os.path.join(install_environment.install_dir, "bin")
                )
    ...
    def execute_step(
        ...
        with lcd(current_dir):
            with settings(warn_only=True):
                ...
                env_file_builder.handle_action_shell_file_paths(action_dict)
                env_file_builder.append_line(
                    name="GEM_PATH", action="prepend_to", value=install_environment.install_dir
                )
                env_file_builder.append_line(
                    name="PATH", action="prepend_to", value=os.path.join(install_environment.install_dir, "bin")
                )
    ...
    def execute_step(
        ...
        with lcd(current_dir):
            with settings(warn_only=True):
                ...
                env_file_builder.append_line(
                    name="PYTHONPATH", action="prepend_to", value=os.path.join(install_environment.install_dir)
                )
                env_file_builder.append_line(
                    name="PATH", action="prepend_to", value=os.path.join(install_environment.install_dir, "bin")
                )
