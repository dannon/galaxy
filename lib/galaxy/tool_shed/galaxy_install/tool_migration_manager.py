...
class ToolMigrationManager:
    def __init__(
        self, app, latest_migration_script_number, tool_shed_install_config, migrated_tools_config, install_dependencies
    ):
        """
                    ...
                    if missing_tool_configs_dict:
                        for proprietary_tool_conf in self.proprietary_tool_confs:
                            ...
                            shutil.copy(
                                ...
                                "%s-pre-stage-%04d" % (proprietary_tool_conf, latest_migration_script_number),
                            )
                        for repository_elem in root:
                            ...
                            if self.__is_valid_repository_tag(repository_elem):
                                ...
                                ) = common_util.get_repository_dependencies(
                                    app, self.tool_shed_url, name, self.repository_owner, changeset_revision
                                )
                                ...
                                ordered_tool_shed_repositories = self.order_repositories_for_installation(
                                    created_tool_shed_repositories, repository_dependencies_dict
                                )
                        ...
                        message = "\nNo tools associated with migration stage %s are defined in your " % str(
    ...
    def install_repository(
        self, repository_elem, tool_shed_repository, install_dependencies, is_repository_dependency=False
    ):
        ...
        if cloned_ok and is_installed:
            ...
            repository_clone_url = os.path.join(
                self.tool_shed_url, "repos", tool_shed_repository.owner, tool_shed_repository.name
            )
            ...
            if not cloned_ok:
                irm.update_tool_shed_repository_status(
                    tool_shed_repository, self.app.install_model.ToolShedRepository.installation_status.CLONING
                )
            ...
            if cloned_ok and not is_installed:
                ...
                irm.update_tool_shed_repository_status(
                    tool_shed_repository, self.app.install_model.ToolShedRepository.installation_status.INSTALLED
                )
            else:
