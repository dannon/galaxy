...
def _iter_manifest_workflows(manifest: Any) -> Iterator[dict[str, Any]]:
    ...
    ``iwc.all_workflows`` assumes the manifest is well formed; here it is
        ...
        entries.append(
            {
                ...
                "trs_tool_id": _as_text(workflow.get("trsID")),
                "trs_version_id": "main",
            }
        )
    return entries
...
def write_projection(path: str, entries: list[dict[str, Any]]) -> None:
    ...
    # Scratch name is unique per writer: web workers and the celery task share
    # this path, and a common tmp name lets one writer publish another's
    # half-written file. os.replace is atomic, so unique names are last-writer-wins.
...
def refresh_projection(path: str, timeout: float = DEFAULT_FETCH_TIMEOUT_SECONDS) -> int | None:
    ...
    already current and nothing was downloaded. Exceptions propagate -- the
    celery task and the background thread each decide how to report them.
    """
    target = Path(path)
    ...
    if current is not None and not iwc.manifest_modified_since(target.stat().st_mtime, timeout):
    ...
    manifest = iwc.download_manifest(timeout)
...
class _Searchable:
    ...
    ``filter_terms`` caps unquoted raw terms but keeps every quoted one, so the
    term count is attacker-controlled on this anonymous endpoint. Rebuilding the
    haystacks per (term, entry) pair made a long quoted query hundreds of
    milliseconds of GIL-held CPU; built once, the term loop is pure substring
    scanning.
    """
