...
from galaxy.workflow.modules import is_tool_module_type, module_factory, ToolModule, WorkflowModuleInjector
...
class WorkflowsManager:
    ...
    def build_invocations_query(
        self, trans, stored_workflow_id=None, history_id=None, user_id=None, include_terminal=True, limit=None
    ):
        """Get invocations owned by the current user."""
        sa_session = trans.sa_session
        invocations_query = sa_session.query(model.WorkflowInvocation).order_by(
            model.WorkflowInvocation.table.c.id.desc()
        )
        ...
        for step in workflow.steps:
            ...
            if len(output_label_duplicate) > 0:
                ...
                upgrade_message_dict["output_label_duplicate"] = (
                    "Ignoring duplicate labels: %s." % output_label_duplicate_string
                )
