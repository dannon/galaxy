...
class JobHandlerQueue(Monitors):
    ...
    def __check_jobs_at_startup(self):
        ...
        for job in jobs_at_startup:
            if not self.app.toolbox.has_tool(job.tool_id, job.tool_version, exact=True):
                ...
                log.debug(
                    "(%s) Job runner assigned but no external ID recorded, adding to the job handler queue" % job.id
                )
        ...
        if state == JOB_READY and "delta" in self.app.job_config.limits.total_walltime:
            jobs_to_check = (
                ...
                .filter(
                    ...
                    - datetime.timedelta(self.app.job_config.limits.total_walltime["window"]),
                    model.Job.state == "ok",
                )
                .all()
            )
    ...
    def __check_user_jobs(self, job, job_wrapper):
        ...
        if job.user:
            ...
            log.warning(
                "Job %s is not associated with a user or session so job concurrency limit cannot be checked." % job.id
            )
        return JOB_READY
