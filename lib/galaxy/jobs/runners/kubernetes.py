...
class KubernetesJobRunner(AsynchronousJobRunner):
    ...
    def setup_volumes(self):
        if self.runner_params.get("k8s_persistent_volume_claims"):
            volume_claims = dict(
                volume.split(":") for volume in self.runner_params["k8s_persistent_volume_claims"].split(",")
            )
        else:
            volume_claims = {}
        mountable_volumes = [
            {"name": claim_name, "persistentVolumeClaim": {"claimName": claim_name}} for claim_name in volume_claims
        ]
        self.runner_params["k8s_mountable_volumes"] = mountable_volumes
        volume_mounts = [
            {"name": claim_name, "mountPath": mount_path} for claim_name, mount_path in volume_claims.items()
        ]
            ...
            if "requests" in resources:
                    ...
                    envs.append(
                        {
                            ...
                            "value": str(ByteSize(requests["memory"]).to_unit("M", as_string=False)),
                        }
                    )
                if "cpu" in requests:
                ...
                if "memory" in limits:
                    envs.append(
                        {
                            "name": "GALAXY_MEMORY_MB",
                            "value": str(ByteSize(limits["memory"]).to_unit("M", as_string=False)),
                        }
                    )
                if "cpu" in limits:
    ...
    def finish_job(self, job_state):
        ...
        if len(jobs.response["items"]) != 1:
            log.warning(
                "More than one job matches selector. Possible configuration error" " in job id '%s'", job_state.job_id
            )
        job = Job(self._pykube_api, jobs.response["items"][0])
        self.__cleanup_k8s_job(job)
