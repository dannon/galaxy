...
class DRMAAJobRunner(AsynchronousJobRunner):
    ...
    def queue_job(self, job_wrapper):
        ...
        if external_runjob_script is None:
            ...
            if pwent is None:
                if not allow_guests:
                    fail_msg = (
                        "User %s is not mapped to any real user, and not permitted to start jobs." % job_wrapper.user
                    )
                    job_wrapper.fail(fail_msg)
                    return
    ...
    def check_watched_items(self):
        ...
        for ajs in self.watched:
            ...
            if state != old_state:
                log.debug(
                    "({}/{}) state change: {}".format(
                        galaxy_id_tag, external_job_id, self.drmaa_job_state_strings[state]
                    )
                )
    ...
    def stop_job(self, job_wrapper):
        ...
        try:
            ...
            log.error(
                "({}/{}) User killed running job, but command execution failed: {}".format(job.id, ext_id, unicodify(e))
            )
        except Exception:
