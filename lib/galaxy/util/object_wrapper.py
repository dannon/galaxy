...
def wrap_with_safe_string(value, no_wrap_classes=None):
    ...
    def __do_wrap(value):
        ...
        wrapped_class_name = "SafeStringWrapper({}:{})".format(
...
class SafeStringWrapper:
    ...
    def __repr__(self):
        return "{} object at {:x} on: {}".format(
            sanitize_lists_to_string(
                self.__class__.__name__, valid_characters=VALID_CHARACTERS, character_map=CHARACTER_MAP
            ),
            id(self),
            sanitize_lists_to_string(
                repr(self.unsanitized), valid_characters=VALID_CHARACTERS, character_map=CHARACTER_MAP
            ),
        )
