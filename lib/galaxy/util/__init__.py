    ...
    if split.password:
        if url.count(split.password) == 1:
            ...
            split = split._replace(
                netloc=split.netloc.replace(f"{split.username}:{split.password}", "%s:********" % split.username)
            )
        ...
        if sanitize:
            for key, value in params.items():
                ...
                if (
                    ...
                    and True
                    not in [
                        key.endswith("|%s" % nonsanitize_parameter) for nonsanitize_parameter in self.NEVER_SANITIZE
                    ]
                ):
