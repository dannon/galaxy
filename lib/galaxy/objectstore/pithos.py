...
class PithosObjectStore(ConcreteObjectStore):
        ...
        **kwargs,
        ...
        if alt_name:
            if not safe_relpath(alt_name):
                log.warning("alt_name would locate path outside dir: {}".format(alt_name))
                raise ObjectInvalid("The requested object is invalid")
        ...
        if not self._exists(obj, **kwargs):
            ...
            if dir_only:
                ...
                rel_path = os.path.join(
                    rel_path, alt_name if alt_name else "dataset_{}.dat".format(self._get_object_id(obj))
                )
    ...
    def _empty(self, obj, **kwargs):
        ...
        if not self._exists(obj, **kwargs):
            raise ObjectNotFound(
                "objectstore.empty, object does not exist: {obj}, " "kwargs: {kwargs}".format(obj=obj, kwargs=kwargs)
            )
    ...
    def _delete(self, obj, **kwargs):
        ...
        try:
            ...
            log.exception("{} delete error".format(self._get_filename(obj, **kwargs)))
        except ClientError as ce:
    ...
    def _get_filename(self, obj, **kwargs):
        ...
        if self._in_cache(path):
            ...
            if not dir_only:
                ...
                return cache_path
        raise ObjectNotFound(
            "objectstore.get_filename, no cache_path: {obj}, " "kwargs: {kwargs}".format(obj=obj, kwargs=kwargs)
        )
