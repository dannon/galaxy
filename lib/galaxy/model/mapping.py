...
model.WorkflowInvocationStep.table = Table(
    ...
    Column(
        "implicit_collection_jobs_id", Integer, ForeignKey("implicit_collection_jobs.id"), index=True, nullable=True
    ),
...
model.WorkflowInvocationToSubworkflowInvocationAssociation.table = Table(
    ...
    Column(
        "subworkflow_invocation_id", Integer, ForeignKey("workflow_invocation.id", name="fk_wfi_swi_swi"), index=True
    ),
...
mapper(
    ...
    properties=dict(
        current=relation(
            ...
            primaryjoin=(
                model.FormDefinition.table.c.form_definition_current_id == model.FormDefinitionCurrent.table.c.id
            ),
        )
    ),
)
...
mapper(
    ...
    properties=dict(
        forms=relation(
            ...
            primaryjoin=(
                model.FormDefinitionCurrent.table.c.id == model.FormDefinition.table.c.form_definition_current_id
            ),
        ),
...
mapper(
    ...
    properties=dict(
        user=relation(
            model.User, primaryjoin=(model.CloudAuthz.table.c.user_id == model.User.table.c.id), backref="cloudauthz"
        ),
        authn=relation(
            model.UserAuthnzToken,
...
mapper(model.DatasetHash, model.DatasetHash.table, properties=dict(dataset=relation(model.Dataset, backref="hashes")))

mapper(
    model.DatasetSource, model.DatasetSource.table, properties=dict(dataset=relation(model.Dataset, backref="sources"))
)
...
mapper(
    ...
    properties=dict(
        ...
        dataset=relation(
            model.HistoryDatasetAssociation,
            primaryjoin=(
                model.ImplicitlyConvertedDatasetAssociation.table.c.hda_id == model.HistoryDatasetAssociation.table.c.id
            ),
            backref="implicitly_converted_parent_datasets",
        ),
    ),
...
mapper(
    ...
    properties=dict(
        ...
        tags=relation(
            model.HistoryTagAssociation, order_by=model.HistoryTagAssociation.table.c.id, backref="histories"
        ),
        annotations=relation(
        ...
        ratings=relation(
            model.HistoryRatingAssociation, order_by=model.HistoryRatingAssociation.table.c.id, backref="histories"
        ),
        average_rating=column_property(
            select([func.avg(model.HistoryRatingAssociation.table.c.rating)]).where(
                model.HistoryRatingAssociation.table.c.history_id == model.History.table.c.id
            ),
            deferred=True,
        ),
        users_shared_with_count=column_property(
            select([func.count(model.HistoryUserShareAssociation.table.c.id)]).where(
                model.History.table.c.id == model.HistoryUserShareAssociation.table.c.history_id
            ),
            deferred=True,
        ),
    ),
...
mapper(
    ...
    properties=dict(
        ...
        active_histories=relation(
            model.History,
            primaryjoin=(
                (model.History.table.c.user_id == model.User.table.c.id) & (not_(model.History.table.c.deleted))
            ),
        ...
        _preferences=relation(
            model.UserPreference, backref="user", collection_class=attribute_mapped_collection("name")
        ),
        # addresses=relation( UserAddress,
        #     primaryjoin=( User.table.c.id == UserAddress.table.c.user_id ) ),
        values=relation(
            model.FormValues, primaryjoin=(model.User.table.c.form_values_id == model.FormValues.table.c.id)
        ),
        api_keys=relation(model.APIKeys, backref="user", order_by=desc(model.APIKeys.table.c.create_time)),
        cloudauthzs=relation(model.CloudAuthz, primaryjoin=model.CloudAuthz.table.c.user_id == model.User.table.c.id),
    ),
)
...
mapper(
    model.Role,
    model.Role.table,
    properties=dict(users=relation(model.UserRoleAssociation), groups=relation(model.GroupRoleAssociation)),
)
...
mapper(
    ...
    properties=dict(
        dataset=relation(model.Dataset, backref="actions"), role=relation(model.Role, backref="dataset_actions")
    ),
)
...
mapper(
    ...
    properties=dict(
        library=relation(model.Library, backref="actions"), role=relation(model.Role, backref="library_actions")
    ),
)
...
mapper(
    model.Library,
    model.Library.table,
    properties=dict(root_folder=relation(model.LibraryFolder, backref=backref("library_root"))),
)
    ...
    properties=dict(
        children=relation(
            ...
            backref=backref(
                "parent",
                primaryjoin=(
                    model.ExtendedMetadataIndex.table.c.extended_metadata_id == model.ExtendedMetadata.table.c.id
                ),
            ),
        )
    ),
...
mapper(
    ...
    properties=dict(
        ...
        template=relation(
            model.FormDefinition,
            primaryjoin=(
                model.LibraryFolderInfoAssociation.table.c.form_definition_id == model.FormDefinition.table.c.id
            ),
        ),
        info=relation(
            model.FormValues,
...
mapper(
    ...
    properties=dict(
        ...
        library_dataset=relation(
            model.LibraryDataset, foreign_keys=model.LibraryDatasetDatasetAssociation.table.c.library_dataset_id
        ),
        # user=relation( model.User.mapper ),
        user=relation(model.User),
        copied_from_library_dataset_dataset_association=relation(
        ...
        extended_metadata=relation(
            model.ExtendedMetadata,
            primaryjoin=(
                model.LibraryDatasetDatasetAssociation.table.c.extended_metadata_id == model.ExtendedMetadata.table.c.id
            ),
        ),
...
mapper(
    ...
    properties=dict(
        ...
        template=relation(
            model.FormDefinition,
            primaryjoin=(
                model.LibraryDatasetDatasetInfoAssociation.table.c.form_definition_id == model.FormDefinition.table.c.id
            ),
        ),
        info=relation(
            model.FormValues,
            primaryjoin=(
                model.LibraryDatasetDatasetInfoAssociation.table.c.form_values_id == model.FormValues.table.c.id
            ),
        ),
    ),
)
...
mapper(
    ...
    properties=dict(
        job=relation(model.Job), dataset=relation(model.HistoryDatasetAssociation, lazy=False, backref="dependent_jobs")
    ),
)
...
mapper(
    ...
    properties=dict(
        job=relation(model.Job), dataset_collection=relation(model.HistoryDatasetCollectionAssociation, lazy=False)
    ),
)
...
mapper(
    ...
    properties=dict(
        job=relation(model.Job), dataset_collection_element=relation(model.DatasetCollectionElement, lazy=False)
    ),
)
...
mapper(
    ...
    properties=dict(
        job=relation(model.Job),
        dataset_collection=relation(model.DatasetCollection, backref="output_dataset_collections"),
    ),
)
...
mapper(
    ...
    properties=dict(
        job=relation(model.Job),
        dataset=relation(model.LibraryDatasetDatasetAssociation, lazy=False, backref="dependent_jobs"),
    ),
)
...
mapper(
    ...
    properties=dict(job=relation(model.Job, backref=backref("container", uselist=False), uselist=False)),
)
...
simple_mapping(
    ...
    elements=relation(
        ...
        primaryjoin=(
            model.DatasetCollection.table.c.id == model.DatasetCollectionElement.table.c.dataset_collection_id
        ),
...
simple_mapping(
    ...
    ldda=relation(
        model.LibraryDatasetDatasetAssociation,
        primaryjoin=(
            model.DatasetCollectionElement.table.c.ldda_id == model.LibraryDatasetDatasetAssociation.table.c.id
        ),
    ),
...
mapper(
    ...
    properties=dict(
        input_step_input=relation(
            ...
            cascade="all",
            primaryjoin=(
                model.WorkflowStepConnection.table.c.input_step_input_id == model.WorkflowStepInput.table.c.id
            ),
        ),
        input_subworkflow_step=relation(
            ...
            primaryjoin=(
                model.WorkflowStepConnection.table.c.input_subworkflow_step_id == model.WorkflowStep.table.c.id
            ),
        ),
...
mapper(
    ...
    properties=dict(
        ...
        owner_tags=relation(
            ...
            primaryjoin=(
                and_(
                    model.StoredWorkflow.table.c.id == model.StoredWorkflowTagAssociation.table.c.stored_workflow_id,
        ...
        average_rating=column_property(
            select([func.avg(model.StoredWorkflowRatingAssociation.table.c.rating)]).where(
                model.StoredWorkflowRatingAssociation.table.c.stored_workflow_id == model.StoredWorkflow.table.c.id
            ),
            deferred=True,
        ),
    ),
...
simple_mapping(
    ...
    implicit_collection_jobs=relation(
        model.ImplicitCollectionJobs, backref=backref("workflow_invocation_step", uselist=False), uselist=False
    ),
    subworkflow_invocation_id=column_property(
        select([(model.WorkflowInvocationToSubworkflowInvocationAssociation.table.c.subworkflow_invocation_id)]).where(
            and_(
                model.WorkflowInvocationToSubworkflowInvocationAssociation.table.c.workflow_invocation_id
...
mapper(
    ...
    properties=dict(
        ...
        annotations=relation(
            model.PageAnnotationAssociation, order_by=model.PageAnnotationAssociation.table.c.id, backref="pages"
        ),
...
mapper(
    ...
    properties=dict(
        ...
        average_rating=column_property(
            select([func.avg(model.VisualizationRatingAssociation.table.c.rating)]).where(
                model.VisualizationRatingAssociation.table.c.visualization_id == model.Visualization.table.c.id
            ),
            deferred=True,
        ),
    ),
...
rating_mapping(
    model.LibraryDatasetCollectionRatingAssociation, libary_dataset_collection=model.LibraryDatasetCollectionAssociation
)
...
model.Job.any_output_dataset_collection_instances_deleted = column_property(  # type: ignore
    exists(
        [model.HistoryDatasetCollectionAssociation.table.c.id],
        and_(
            model.Job.table.c.id == model.JobToOutputDatasetCollectionAssociation.table.c.job_id,
            model.HistoryDatasetCollectionAssociation.table.c.id
