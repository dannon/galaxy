    ...
    def setup_external_metadata(
        ...
        for name, dataset in datasets_dict.items():
            ...
            _initialize_metadata_inputs(
                dataset, _metadata_path, tmp_dir, kwds, real_metadata_object=real_metadata_object
            )
        ...
        if self.write_object_store_conf:
            ...
            with DirectoryModelExportStore(
                export_directory, for_edit=True, serialize_dataset_objects=True
            ) as export_store:
    ...
    def _get_output_filenames_by_dataset(self, dataset, sa_session):
        if isinstance(dataset, galaxy.model.HistoryDatasetAssociation):
            return (
                ...
                .filter_by(job_id=self.job_id, history_dataset_association_id=dataset.id, is_valid=True)
                .first()
            )  # there should only be one or None
        elif isinstance(dataset, galaxy.model.LibraryDatasetDatasetAssociation):
            return (
                sa_session.query(galaxy.model.JobExternalOutputMetadata)
                .filter_by(job_id=self.job_id, library_dataset_dataset_association_id=dataset.id, is_valid=True)
                .first()
            )  # there should only be one or None
        return None
    ...
    def setup_external_metadata(
        ...
        if include_command:
            ...
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=".py", dir=tmp_dir, prefix="set_metadata_", delete=False
            ) as temp:
