    ...
    for context_value in other_values.values():
        ...
        for v in util.listify(context_value):
            if isinstance(v, trans.app.model.HistoryDatasetAssociation) and (
                (hasattr(v, "state") and v.state != galaxy.model.Dataset.states.OK) or hasattr(v, "implicit_conversion")
            ):
                return True
    return False
    ...
    def from_json(self, value, trans, other_values=None, require_legal_value=True):
        ...
        if isinstance(value, list):
            ...
            if value in legal_values:
                ...
                raise ParameterValueError(
                    "an invalid option ({!r}) was selected (valid options: {})".format(value, ",".join(legal_values)),
                    self.name,
                    value,
                    is_dynamic=self.is_dynamic,
                )
    ...
    def from_json(self, value, trans, other_values=None):
        ...
        for val in value:
            if val not in legal_values:
                raise ParameterValueError(
                    "an invalid option ({!r}) was selected (valid options: {})".format(val, ",".join(legal_values)),
                    self.name,
                    val,
                )
            rval.append(val)
...
class BaseDataToolParameter(ToolParameter):
    ...
    def to_python(self, value, app):
        ...
        if isinstance(value, str) and value.find(",") > -1:
            ...
            return app.model.context.query(app.model.HistoryDatasetCollectionAssociation).get(
                int(value[len("hdca:") :])
            )
        else:
    ...
    def from_json(self, value, trans, other_values=None):
        ...
        if isinstance(value, list):
            ...
            for single_value in value:
                if isinstance(single_value, dict) and "src" in single_value and "id" in single_value:
                    if single_value["src"] == "hda":
                        ...
                        decoded_id = trans.security.decode_id(single_value["id"])
                        rval.append(
                            trans.sa_session.query(trans.app.model.HistoryDatasetCollectionAssociation).get(decoded_id)
                        )
                    elif single_value["src"] == "ldda":
                        decoded_id = trans.security.decode_id(single_value["id"])
                        rval.append(
                            trans.sa_session.query(trans.app.model.LibraryDatasetDatasetAssociation).get(decoded_id)
                        )
                    else:
            ...
            for decoded_id in decoded_ids:
                ...
                rval.append(hdca)
        elif isinstance(value, trans.app.model.HistoryDatasetCollectionAssociation) or isinstance(
            value, trans.app.model.DatasetCollectionElement
        ):
            rval = value
        else:
            ...
            if len(value) > 0:
                ...
                if isinstance(value, dict) and "src" in value and "id" in value:
                    if value["src"] == "hdca":
                        ...
                        rval = trans.sa_session.query(trans.app.model.DatasetCollectionElement).get(
                            trans.security.decode_id(value["id"])
                        )
        elif isinstance(value, str):
        ...
        for item in value:
            if isinstance(item, app.model.LibraryDatasetDatasetAssociation):
                ...
                if isinstance(item, dict):
                    ...
                    lst = []
                    break
                lda = app.model.context.query(app.model.LibraryDatasetDatasetAssociation).get(
