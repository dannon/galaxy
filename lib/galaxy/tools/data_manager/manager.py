    ...
    def process_result(self, out_data):
        ...
        for data_table_name in self.data_tables.keys():
            ...
            if data_table is None:
                log.error(
                    'The data manager "{}" returned an unknown data table "{}" with new entries "{}". These entries will not be created. Please confirm that an entry for "{}" exists in your "{}" file.'.format(
            ...
            if not isinstance(data_table, SUPPORTED_DATA_TABLE_TYPES):
                log.error(
                    'The data manager "{}" returned an unsupported data table "{}" with type "{}" with new entries "{}". These entries will not be created. Please confirm that the data table is of a supported type ({}).'.format(
