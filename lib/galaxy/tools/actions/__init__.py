            ...
            def process_dataset(data, formats=None):
                ...
                if collection_info and collection_info.is_mapped_over(input_name):
                    ...
                    if not trans.app.security_agent.can_access_dataset(current_user_roles, data.dataset):
                        raise ItemAccessibilityException(
                            "User does not have permission to use a dataset (%s) provided for input." % data.id
                        )
            ...
            if isinstance(input, DataToolParameter):
                            ...
                            if not new_data or new_data.datatype.matches_any(conversion_datatypes):
                                ...
                                raise Exception(
                                    "A path for explicit datatype conversion has not been found: {} --/--> {}".format(
                                        input_datasets[prefix + input.name + str(i + 1)].extension,
                ...
                if conversion_required:
                    collection_type_description = (
                        trans.app.dataset_collections_service.collection_type_descriptions.for_collection_type(
