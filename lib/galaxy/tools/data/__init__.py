...
class ToolDataTableManager:
    ...
    def add_new_entries_from_config_file(
        ...
        try:
            ...
            error_message = "Error attempting to parse file {}: {}".format(
                str(os.path.split(config_filename)[1]), util.unicodify(e)
            )
...
class ToolDataTable:
    ...
    def add_entries(
        self, entries, allow_duplicates=True, persist=False, persist_on_error=False, entry_source=None, **kwd
    ):
    ...
    def merge_tool_data_table(
        self, other_table, allow_duplicates=True, persist=False, persist_on_error=False, entry_source=None, **kwd
    ):
        raise NotImplementedError("Abstract method")
    ...
    def _add_entry(self, entry, allow_duplicates=True, persist=False, persist_on_error=False, entry_source=None, **kwd):
        ...
        if persist and (not is_error or persist_on_error):
            ...
            if filename is None:
                ...
                log.error(
                    "Unable to determine filename for persisting data table '%s' values: '%s'.", self.name, fields
                )
                is_error = True
            else:
