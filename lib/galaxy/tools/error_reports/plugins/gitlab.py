...
class GitLabPlugin(BaseGitPlugin):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            if gitlab is None:
                raise Exception(
                    "GitLab error reporting plugin is configured, but gitlab is not installed. Please install python-gitlab."
                )
        ...
        if self.gitlab:
            ...
            try:
                ...
                try:
                    ...
                    log.warning(
                        "GitLab error reporting - Repository '%s' doesn't exist, using default repository."
                ...
                if len(gl_project.commits.list()) > 0:
                    ...
                    if gl_useremail not in self.git_username_id_cache:
                        ...
                        if len(gl_emailquery) > 0:
                            log.debug("GitLab error reporting - Last Committer user ID: %d" % gl_emailquery[0].get_id())
                ...
                if error_title not in self.issue_cache[issue_cache_key]:
                    ...
                    if not self.gitlab_new_issue_on_closed:
                        ...
                        self._append_issue(
                            issue_cache_key, error_title, error_message, gitlab_urlencodedpath=gitlab_urlencodedpath
                        )
                    else:
                        self._open_issue(
                            error_message, error_title, gitlab_projecturl, gl_project, gl_userid, issue_cache_key
