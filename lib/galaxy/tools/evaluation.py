        ...
        def wrap_input(input_values, input):
            ...
            if isinstance(input, DataToolParameter) and input.multiple:
                ...
                for conversion_name, conversion_extensions, conversion_datatypes in input.conversions:
                    ...
                    direct_match, conv_ext, converted_dataset = input_values[input.name].find_conversion_destination(
        ...
        for name, data in input_datasets.items():
            ...
            if data and param_dict_value is None:
                ...
                wrappers = find_instance_nested(
                    param_dict, instances=(DatasetFilenameWrapper, DatasetListWrapper), match_key=name
                )
                if len(wrappers) == 1:
                    wrapper = wrappers[0]
        ...
        for ep in getattr(self.tool, "ports", []):
            ...
            for key in "port", "name", "url", "requires_domain":
                ...
                if val is not None and not isinstance(val, bool):
                    val = fill_template(
                        val, context=param_dict, python_template_version=self.tool.python_template_version
                    )
                    clean_val = []
