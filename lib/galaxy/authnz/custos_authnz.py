...
class CustosAuthnz(IdentityProvider):
            ...
            if not user:
                ...
                if existing_user:
                    ...
                    if (
                        ...
                        message = (
                            "There already exists a user with email %s.  To associate this external login, you must first be logged in as that existing account."
                            % email
                        )
                    ...
                    login_redirect_url = (
                        login_redirect_url + "root/login?confirm=true&custos_token=" + json.dumps(token)
                    )
    ...
    def _fetch_token(self, oauth2_session, trans):
        ...
        return oauth2_session.fetch_token(
            ...
            headers={
                "Authorization": "Basic %s" % util.unicodify(base64.b64encode(util.smart_str(clientIdAndSec)))
            },  # for custos
