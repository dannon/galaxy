...
class AuthnzManager:
    ...
    def callback(self, provider, state_token, authz_code, trans, login_redirect_url, idphint=None):
        try:
            ...
            msg = "The following error occurred when handling callback from `{}` identity provider: " "{}".format(
                provider, str(e)
            )
            log.exception(msg)
    ...
    def logout(self, provider, trans, post_logout_redirect_url=None):
        ...
        try:
            ...
            msg = "The following error occurred when logging out from `{}` identity provider: " "{}".format(
                provider, str(e)
            )
            log.exception(msg)
            return False, msg, None
