...
class GalaxyAppConfiguration(BaseAppConfiguration, CommonConfigurationMixin):
    ...
    def _process_config(self, kwargs):
        ...
        self.container_image_cache_path = self._in_data_dir(
            kwargs.get("container_image_cache_path", "container_images")
        )
        if not os.path.exists(self.container_image_cache_path):
            self.container_image_cache_path = self._in_root_dir(
                kwargs.get("container_image_cache_path", self._in_data_dir("container_cache"))
            )
        ...
        if "amqp_internal_connection" in kwargs:
            ...
            self.amqp_internal_connection = "sqlalchemy+sqlite:///%s?isolation_level=IMMEDIATE" % self._in_data_dir(
                "control.sqlite"
            )
        ...
        try:
            ...
            if self.is_set("user_preferences_extra_conf_path"):
                log.warning(
                    "Config file (%s) could not be found or is malformed." % self.user_preferences_extra_conf_path
                )
    ...
    def _configure_dataset_storage(self):
        ...
        if self.is_set("object_store_store_by"):
            assert self.object_store_store_by in [
                ID,
                UUID,
...
class ConfiguresGalaxyMixin:
    ...
    def _configure_tool_config_files(self):
        ...
        if (
            self.config.check_migrate_tools or os.path.exists(self.config.migrated_tools_config)
