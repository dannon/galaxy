...
def setup_galaxy_config(
    ...
    if not use_shared_connection_for_amqp:
        config["amqp_internal_connection"] = "sqlalchemy+sqlite:///%s?isolation_level=IMMEDIATE" % os.path.join(
            tmpdir, "control.sqlite"
        )
...
def launch_uvicorn(gx_app, webapp_factory, kwargs, prefix=DEFAULT_CONFIG_PREFIX, config_object=None):
    ...
    gx = webapp_factory(
        kwargs["global_conf"], app=gx_app, use_translogger=False, static_enabled=True, register_shutdown_at_exit=False
    )
    ...
    return EmbeddedServerWrapper(gx_app, server, name, host, port)
