...
class JobsApiTestCase(ApiTestCase, TestsTools):
    ...
    def test_report_error(self):
        with self.dataset_populator.test_history() as history_id:
            ...
            response = self._post("jobs/%s/error" % job_id, data={"dataset_id": dataset_id})
    ...
    def test_report_error_anon(self):
        ...
        payload = json.dumps({"tool_id": "detect_errors_aggressive", "inputs": {"error_bool": "true"}})
        run_response = requests.post("%s/tools" % self.galaxy_interactor.api_url, data=payload, cookies=cookies).json()
        job_id = run_response["jobs"][0]["id"]
        dataset_id = run_response["outputs"][0]["id"]
        response = requests.post(
            f"{self.galaxy_interactor.api_url}/jobs/{job_id}/error",
    ...
    def test_resume_job(self, history_id):
        ...
        undelete_response = self._put(
            "histories/{}/contents/{}".format(history_id, hda1["id"]), data=json.dumps({"deleted": False})
        )
    ...
    def test_search_handle_identifiers(self, history_id):
        ...
        dataset_update_response = self._put(
            f"histories/{history_id}/contents/{dataset_id}", data=dict(name="Renamed Test Dataset")
        )
