...
class UsersApiTestCase(ApiTestCase):
    ...
    def test_favorites(self):
        ...
        url = self._api_url(
            "users/%s/favorites/tools/madeuptoolthatdoes/not/exist/in/favs" % user["id"],
        ...
        url = self._api_url(
            "users/%s/favorites/workflows/1as5das5das56d465" % user["id"], params=dict(key=self.master_api_key)
        )
        delete_response = delete(url)
