...
class User(BaseUser):
    ...
    def login(self, trans, refresh_frames=None, **kwd):
        ...
        if user:
            ...
            if trans.response.status == 400:
                ...
                message = "{}<br>Click <a href='{}'>here</a> to change your password.".format(
        ...
        if subscribe_checked:
            ...
            if trans.app.config.smtp_server is None:
                ...
                try:
                    ...
                    status = "warning"
                    message = (
                        "Now logged in as " + user.email + ". However, subscribing to the mailing list has failed."
                    )
        if status != "error":
            if not is_admin:
                # The handle_user_login() method has a call to the history_set_default_permissions() method
            ...
            if trans.app.config.user_activation_on:
                ...
                if is_activation_sent:
                    message = 'Now logged in as {}.<br>Verification email has been sent to your email address. Please verify it by clicking the activation link in the email.<br>Please check your spam/trash folder in case you cannot find the message.<br><a target="_top" href="{}">Return to the home page.</a>'.format(
            ...
            message = 'Now logged in as {}.<br><a target="_top" href="{}">Return to the home page.</a>'.format(
                escape(user.email), url_for("/")
            )
    ...
    def __validate(self, trans, email, password, confirm, username):
        if username in ["repos"]:
            return (
                "The term '%s' is a reserved word in the Tool Shed, so it cannot be used as a public user name."
                % username
            )
        message = "\n".join(
            (
