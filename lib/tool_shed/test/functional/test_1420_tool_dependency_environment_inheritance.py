...
package_bzlib_repository_description = (
    "Contains a tool dependency definition that downloads and compiles version 1.0 of the bzlib library."
)
...
package_numpy_repository_description = (
...
package_lapack_repository_long_description = (
    f"{package_lapack_repository_name}: {package_lapack_repository_description}"
)
...
class TestEnvironmentInheritance(ShedTwillTestCase):
    ...
    def test_0005_create_lapack_repository(self):
        ...
        repository = self.get_or_create_repository(
            name=package_lapack_repository_name,
        ...
        tool_dependency_path = self.generate_temp_path(
            "1420_tool_dependency", additional_paths=["package_lapack_3_4_1420"]
        )
        precompiled_binary_tarball = self.get_filename("1420_files/binary_tarballs/lapack.tar")
        edited_tool_dependency_filename = self.get_filename(
            filepath=tool_dependency_path, filename="tool_dependencies.xml"
        )
        original_tool_dependency = self.get_filename("1420_files/package_lapack_3_4_1420/tool_dependencies.xml")
        tool_dependency_definition = (
            open(original_tool_dependency).read().replace("__PATH__", precompiled_binary_tarball)
        )
    ...
    def test_0010_create_atlas_repository(self):
        ...
        category = self.test_db_util.get_category_by_name(category_name)
        repository = self.get_or_create_repository(
            name=package_atlas_repository_name,
        ...
        # actual system path where the binary tarball is found.
        tool_dependency_path = self.generate_temp_path(
            "1420_tool_dependency", additional_paths=["package_atlas_3_10_1420"]
        )
    ...
    def test_0015_create_bzlib_repository(self):
        ...
        category = self.test_db_util.get_category_by_name(category_name)
        repository = self.get_or_create_repository(
            name=package_bzlib_repository_name,
        ...
        # actual system path where the binary tarball is found.
        tool_dependency_path = self.generate_temp_path(
            "1420_tool_dependency", additional_paths=["package_bzlib_1_0_1420"]
        )
    ...
    def test_0020_create_boost_repository(self):
        ...
        category = self.test_db_util.get_category_by_name(category_name)
        repository = self.get_or_create_repository(
            name=package_boost_repository_name,
        ...
        # actual system path where the binary tarball is found.
        tool_dependency_path = self.generate_temp_path(
            "1420_tool_dependency", additional_paths=["package_boost_1_53_1420"]
        )
    ...
    def test_0025_create_numpy_repository(self):
        ...
        category = self.test_db_util.get_category_by_name(category_name)
        repository = self.get_or_create_repository(
            name=package_numpy_repository_name,
        ...
        # actual system path where the binary tarball is found.
        tool_dependency_path = self.generate_temp_path(
            "1420_tool_dependency", additional_paths=["package_numpy_1_7_1420"]
        )
    ...
    def test_0030_create_rdkit_repository(self):
        ...
        category = self.test_db_util.get_category_by_name(category_name)
        repository = self.get_or_create_repository(
            name=package_rdkit_repository_name,
        ...
        # actual system path where the binary tarball is found.
        tool_dependency_path = self.generate_temp_path(
            "1420_tool_dependency", additional_paths=["package_rdkit_2012_12_1420"]
        )
    ...
    def test_0035_install_rdkit_2012_12_repository(self):
        ...
        post_submit_strings_displayed = [
            "package_rdkit_2012_12_1420",
            "package_atlas_3_10_1420",
            "package_bzlib_1_0_1420",
            "package_numpy_1_7_1420",
            "package_lapack_3_4_1420",
            "package_boost_1_53_1420",
        ]
    ...
    def test_0040_verify_env_sh_contents(self):
        ...
        """
        package_rdkit_repository = self.test_db_util.get_installed_repository_by_name_owner(
            "package_rdkit_2012_12_1420", common.test_user_1_name
        )
        package_numpy_repository = self.test_db_util.get_installed_repository_by_name_owner(
            "package_numpy_1_7_1420", common.test_user_1_name
        )
        package_boost_repository = self.test_db_util.get_installed_repository_by_name_owner(
            "package_boost_1_53_1420", common.test_user_1_name
        )
        rdkit_env_sh = self.get_env_sh_path(
            tool_dependency_name="rdkit", tool_dependency_version="2012_12_1", repository=package_rdkit_repository
        )
        numpy_tool_dependency_path = self.get_tool_dependency_path(
            tool_dependency_name="numpy", tool_dependency_version="1.7.1", repository=package_numpy_repository
        )
        boost_tool_dependency_path = self.get_tool_dependency_path(
            tool_dependency_name="boost", tool_dependency_version="1.53.0", repository=package_boost_repository
        )
        rdkit_env_file_contents = open(rdkit_env_sh).read()
        if (
            numpy_tool_dependency_path not in rdkit_env_file_contents
