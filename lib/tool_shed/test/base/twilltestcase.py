...
class ShedTwillTestCase(DrivenFunctionalTestCase):
    ...
    def check_repository_invalid_tools_for_changeset_revision(
        ...
        repository_metadata = self.get_repository_metadata_by_changeset_revision(repository, changeset_revision)
        metadata = repository_metadata.metadata
        assert "invalid_tools" in metadata, (
            "Metadata for changeset revision %s does not define invalid tools" % changeset_revision
        )
    ...
    def deactivate_repository(self, installed_repository, strings_displayed=None, strings_not_displayed=None):
        ...
        response = requests.delete(
            self.galaxy_url + "/api/tool_shed_repositories/" + encoded_id,
    ...
    def display_installed_jobs_list_page(
        ...
        if data_manager_names:
            ...
            for data_manager_name in data_manager_names:
                assert data_manager_name in data_managers, (
                    "The requested Data Manager '%s' was not found in repository metadata." % data_manager_name
                )
        else:
    ...
    def install_repository(
        ...
        **kwd,
        ...
        if len(submit_button) == 0:
            ...
            assert len(submit_button) == 1, "Expected to find a single submit button, found {} ({})".format(
                len(submit_button), ",".join(submit_button)
            )
    ...
    def reset_installed_repository_metadata(self, repository):
        ...
        response = requests.post(
            self.galaxy_url + "/api/tool_shed_repositories/reset_metadata_on_selected_installed_repositories",
            ...
            for required_data_table_entry in required_data_table_entries:
                ...
                if "name" in table_elem.attrib and table_elem.attrib["name"] == required_data_table_entry:
                    ...
                    assert file_path is not None, (
                        'The "path" attribute is missing for the %s entry.' % required_data_table_entry
                    )
                    ...
                    assert galaxy_tool_data_dir is not None, (
                        "The hard-coded Galaxy tool-data directory is missing for the %s entry."
                        % required_data_table_entry
                    )
                    ...
                    assert os.path.isfile(configured_file_location), (
                        'The expected copied file "%s" is missing.' % configured_file_location
                    )
