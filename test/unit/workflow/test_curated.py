...
call it can make (``iwc.download_manifest``) is monkeypatched in the two tests
that exercise the background refresher.
"""
...
def test_project_manifest_maps_the_remaining_card_fields() -> None:
    ...
    assert velocyto["trs_server"] == "dockstore"
    assert velocyto["trs_tool_id"] == "#workflow/github.com/iwc-workflows/velocyto/main"
    assert velocyto["trs_version_id"] == "main"
...
def test_load_projection_serves_from_cache_until_the_mtime_changes(projection_path: str, monkeypatch) -> None:
    parses = []
    real_loads = json.loads

    def counting_loads(*args, **kwds):
        parses.append(args[0])
        return real_loads(*args, **kwds)

    monkeypatch.setattr(curated.json, "loads", counting_loads)
    ...
    assert curated.load_projection(projection_path) == first
    assert len(parses) == 1, "an unchanged file must not be re-parsed"
...
def test_request_background_refresh_never_downloads_on_the_calling_thread(projection_path: str, monkeypatch) -> None:
    ...
    monkeypatch.setattr(curated.iwc, "download_manifest", blocking_download)
...
def test_refresh_projection_skips_the_download_when_upstream_is_unchanged(projection_path: str, monkeypatch) -> None:
    ...
    monkeypatch.setattr(curated.iwc, "manifest_modified_since", lambda mtime, timeout=None: False)
    monkeypatch.setattr(curated.iwc, "download_manifest", lambda timeout: downloads.append(timeout) or [])
...
def test_refresh_projection_downloads_when_upstream_moved(projection_path: str, monkeypatch) -> None:
    ...
    monkeypatch.setattr(curated.iwc, "manifest_modified_since", lambda mtime, timeout=None: True)
    monkeypatch.setattr(curated.iwc, "download_manifest", lambda timeout: SAMPLE_MANIFEST)
...
def test_refresh_projection_refuses_to_replace_a_good_catalog_with_an_empty_one(
    ...
    monkeypatch.setattr(curated.iwc, "manifest_modified_since", lambda mtime, timeout=None: True)
    monkeypatch.setattr(curated.iwc, "download_manifest", lambda timeout: [])
...
def test_search_curated_handles_many_quoted_terms() -> None:
    """filter_terms caps unquoted raw terms but keeps every quoted one.

    The term count is therefore attacker-controlled on an anonymous endpoint,
    so matching must not rebuild per-entry haystacks for each term.
    """
    entries = curated.project_manifest(SAMPLE_MANIFEST)
