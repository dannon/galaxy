...
``galaxy.agents.iwc.download_manifest`` with functions that raise, so any code
...
def _catalog_entry(slug: str, name: str, tags: list[str], updated: str) -> dict[str, Any]:
    ...
    return {
        ...
        "trs_tool_id": f"#workflow/github.com/iwc-workflows/{slug}/main",
        "trs_version_id": "main",
    }
...
class TestCuratedWorkflowsLocal(_CuratedWorkflowsTestCase):
    """``curated_workflow_owners`` set: the tab lists those accounts' published workflows."""
    ...
    def handle_galaxy_config_kwds(cls, config):
        ...
        config["curated_workflows_enabled"] = True
    ...
    def test_local_source_and_rows(self):
        ...
        for workflow in index["workflows"]:
            ...
            assert workflow["trs_tool_id"] is None
...
class TestCuratedWorkflowsCatalog(_CuratedWorkflowsTestCase):
    """``curated_workflow_owners`` empty: the tab serves the on-disk IWC projection."""
    ...
    def handle_galaxy_config_kwds(cls, config):
        ...
        config["curated_workflows_enabled"] = True
        config["curated_workflow_owners"] = ""
    ...
    def test_catalog_source_and_rows(self):
        ...
        for workflow in index["workflows"]:
            ...
            assert workflow["trs_server"] == "dockstore"
            assert workflow["trs_version_id"] == "main"
...
class TestCuratedWorkflowsDisabled(_CuratedWorkflowsTestCase):
    @classmethod
    def handle_galaxy_config_kwds(cls, config):
        super().handle_galaxy_config_kwds(config)
        config["curated_workflows_enabled"] = False

    def test_disabled_returns_config_does_not_allow(self):
    ...
    def test_disabled_anonymous_returns_config_does_not_allow(self):
